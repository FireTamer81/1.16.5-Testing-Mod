...
public abstract class TestAnimatedGeoModel<T extends IAnimatable & IAnimationTickable> extends AnimatedGeoModel<T> {

    public TestAnimatedGeoModel() {}

    public GeckoTestBone getGeckoTestBone(String boneName) {
        IBone bone = this.getBone(boneName);
        return (GeckoTestBone) bone;
    }
    ...
    public void setLivingAnimations(T entity, Integer uniqueID, @Nullable AnimationEvent customPredicate) {
        ...
        if (manager.startTick == null) {
            //.getDeltaFrameTime() was .getRenderPartialTicks()
            //If this doesn't work as intended, revert and try again.
            manager.startTick = (double) (entity.tickTimer() + Minecraft.getInstance().getDeltaFrameTime());    // Set start ticks when animation starts playing
        }

        if (!Minecraft.getInstance().isPaused() || manager.shouldPlayWhilePaused) {
            //.getDeltaFrameTime() was .getRenderPartialTicks()
            //If this doesn't work as intended, revert and try again.
            manager.tick = (entity.tickTimer() + Minecraft.getInstance().getDeltaFrameTime());
