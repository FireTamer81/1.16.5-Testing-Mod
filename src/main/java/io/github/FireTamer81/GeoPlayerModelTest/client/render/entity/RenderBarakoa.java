...
import io.github.FireTamer81.GeoPlayerModelTest.client.model.entity.ModelBarakoa;
...
public class RenderBarakoa extends MobRenderer<EntityBarakoa, ModelBarakoa<EntityBarakoa>> {
    ...
    static {
        for (MaskType mask : MaskType.values()) {
            TEXTURES.put(mask, new ResourceLocation(TestModMain.MODID, "textures/entity/barakoa_" + mask.name + (ConfigHandler.CLIENT.oldBarakoaTextures.get() ? "_old" : "") + ".png"));
            WADOO_TEXTURES.put(mask, new ResourceLocation(TestModMain.MODID, "textures/entity/barakoa_" + mask.name + "_wadoo.png"));
        }
    }
